...
local kanastatus = Class(function(self, inst)
    self.inst = inst
    self.level = 0
    self.exp = 0
    self.expold = 0
...
function kanastatus:OnLoad(data)
...
function kanastatus:DoDeltaExp(delta)
    if delta > 0 then
	...
	self.inst:PushEvent("DoDeltaExpKANA")
    ---self.inst:PushEvent("DoDeltaExpCARNEY")
end

function kanastatus:powerready()
	self.inst:PushEvent("powerready")
end

function kanastatus:spelldone()
